...
int main(void) {
   ...
   double scalar;
   if (my_rank == 0) {
      printf("Enter the scalar to multiply with the vectors: ");
      scanf("%lf", &scalar);
   }
   MPI_Bcast(&scalar, 1, MPI_DOUBLE, 0, comm);

   Parallel_scalar_mult(scalar, local_x, local_z, local_n);
   Print_vector(local_z, local_n, n, "Scalar multiplied vector x:", my_rank, comm);

   Parallel_scalar_mult(scalar, local_y, local_z, local_n);
   Print_vector(local_z, local_n, n, "Scalar multiplied vector y:", my_rank, comm);

   Parallel_dot_product(local_x, local_y, local_z, local_n);
   if (my_rank == 0) {
      double dot_product = 0;
      for (int i = 0; i < local_n; i++)
         dot_product += local_z[i];
